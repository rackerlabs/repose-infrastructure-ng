...
4. Run the client `bootstrap.sh` from this repository
5. The last step of the `bootstrap.sh` fires up the puppet agent.
    1. There may be some additional steps indicated when the `bootstrap.sh` completes; read them *all* and do in order.
    2. The client's host certificate will also most likely need to be signed on the master:
        `puppet cert clean <FQHN>`
    3. Then the puppet agent will need to be executed on the client again:
        `puppet agent -t`
6. If the new box is a Jenkins slave, then update Jenkins' node list.
